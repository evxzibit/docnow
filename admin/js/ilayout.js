...
//--- Runs a sequence of steps to change the opacity
	...
	if (opacStart > opacEnd){
		for (i = opacStart; i >= opacEnd; i--){
			setTimeout ("_changeOpac(" + i + ", '" + id + "')", (timer * speed)); timer++;
		}
	} else
	if (opacStart < opacEnd){
		for (i = opacStart; i <= opacEnd; i++){
...
//--- Changes the opacity for one frame
