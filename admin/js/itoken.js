...
function tokenizer (f, u, p) {
	...
	//--- Creates the elements required for a tokenizer
		...
		for (i = 0; i < this.formField.options.length; i++) {
			if (this.formField.options[i].getAttribute ('display') != 'none') {
		...
		this.suggestionLayer = new suggestionList (_$ (this.inputField), this.callBack.replace (/{field}/g, this.holderList).replace (/{selection}/g, this.currentSelection), 1, {'topTip': false, 'shadow': false, 'customStyle': 'border: 1px solid #999; border-top: 0px; background-color: #ffffff;' + (this.parameters['customStyle'] ? ' ' + this.parameters['customStyle'] : ''), 'scrollTopObj': this.parameters['scrollTopObj'], 'scrollLeftObj': this.parameters['scrollLeftObj']});
	...
	//--- Removes a node from the unordered list and select list
		...
		this.suggestionLayer.hideSuggestions ();
	...
	//--- Replaces the callback with an updated list
		...
		if (this.parameters['callOnChange']) {
			this.parameters['callOnChange'] (this);
		}
