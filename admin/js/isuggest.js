				...
				case 200:	//--- Display response text
							...
							if (tagList.length) {
								...
								for (i = 0; i < tagList.length; i++) {
									...
									tagLinks += "<div data-id=\"" + sanitizeSuggestion (id) + "\" data-val=\"" + sanitizeSuggestion (value) + "\" class=\"suggestStandard\" style=\"padding: 5px; cursor: pointer;\" valign=\"top\" onmouseover=\"if (top.lastSuggestionList.lastSelected != null) { top.lastSuggestionList.lastSelected.style.backgroundColor = '#ffffff'; top.lastSuggestionList.lastSelected = null; } top.lastSuggestionList.lastSelected = this; this.style.backgroundColor = '#ccffcc';\" onmouseout=\"if (top.lastSuggestionList.lastSelected != null) { top.lastSuggestionList.lastSelected.style.backgroundColor = '#ffffff'; top.lastSuggestionList.lastSelected = null; } this.style.backgroundColor = '#ffffff'\" onclick=\"if (top.lastSuggestionList.lookupType == 0) { top.lastSuggestionList.replaceWord ('" + sanitizeSuggestion (value) + "'); " + (id ? "if (top.lastSuggestionList.parameters['setValue']) { top.lastSuggestionList.parameters['setValue'].value = '" + sanitizeSuggestion (id) + "'; }" : "") + (callback ? callback + " ('" + which.formField.id + "', '" + sanitizeSuggestion (value) + "', '" + sanitizeSuggestion (id) + "'); " : "") + " } else if (top.lastSuggestionList.lookupType == 1) { top.lastSuggestionList.replacePhrase ('" + sanitizeSuggestion (value) + "'); " + (id ? "if (top.lastSuggestionList.parameters['setValue']) { top.lastSuggestionList.parameters['setValue'].value = '" + sanitizeSuggestion (id) + "'; }" : "") + (callback ? callback + " ('" + which.formField.id + "', '" + sanitizeSuggestion (value) + "', '" + sanitizeSuggestion (id) + "'); " : "") + " } else { " + (location ? "window.location = '" + location + "';" : "") + " } return false\"><div style=\"width: " + (document.all ? (fieldWidth - 10) : (fieldWidth - 20)) + "px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; -o-text-overflow: ellipsis\" title=\"" + sanitizeSuggestion (value) + "\">" + (outputFormat == 'html' ? tagList[i].firstChild.nodeValue.toString () : sanitizeSuggestion (tagList[i].firstChild.nodeValue.toString ()).replace (new RegExp ("(" + whichWord + ")", "ig"), "<b>$1</b>")) + "</div></div>";
								}
								tagLinks += "</div>";
							}
							if (tagLinks) {
								...
								which.suggestionLayer.style.top = (textAreaObjPos[1] + document.getElementById (whichField).offsetHeight + (which.parameters['scrollTopObj'] ? 0 - _$ (which.parameters['scrollTopObj']).scrollTop : 0)) + 'px';
								which.suggestionLayer.style.left = (textAreaObjPos[0] + (which.parameters['scrollLeftObj'] ? 0 - _$ (which.parameters['scrollLeftObj']).scrollLeft : 0)) + 'px';
