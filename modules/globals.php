...
//--- Define your custom fusion processor here
	...
	switch ($Tag['name']) {
		...
		case 'IMAGE':		return ($Tag['attributes']['class'] ? str_replace ("<img ", "<img class=\"".$Tag['attributes']['class']."\" ", $Tag['content']) : $Tag['content']);
...
function AuthoriseLDAP ($Eml, $Pwd, $Dest_URL) {
//--- Allows the LDAP directory to authenticate the user rather than Ensight

	if ($Eml == 'joesoap7') {
		return array ("unique_id" => "12345", "full_name" => "Joe Soap 456", "status" => PROFILE_ACTIVE);
	} else {
		return array ("error" => "Could not authorise");
	}

}

function CustomProductSegment () {
//--- A custom query on the database

	return "SELECT Profile_ID FROM Transactions WHERE PurchaseCategory_STRING = 'DVDs'";

}

$CustomSegments = array (
	array ("text" => "Custom Products", "function" => "CustomProductSegment")
);

function callbackTeamFan ($RelatedToObjectType, $RelatedToObject_ID) {
//--- Looks up and returns a team pulse

	//--- Look up team referenced
	if ($RelatedToObjectType != 'teamfan') {
		return false;
	}

	$SQL = "SELECT Team_ID, TeamName_STRING, TeamPhoto_STRING FROM MTNfootball_Teams WHERE Team_ID = '".$RelatedToObject_ID."'";
	$XQuery = QueryDB ($SQL);
	$TeamResult = ReadFromDB ($XQuery);

	$SQL = "SELECT Item_ID FROM MTNfootball_Catalog_Layouts WHERE LayoutType_CHAR = 'TP' AND Team_ID = '".$RelatedToObject_ID."'";
	$XQuery = QueryDB ($SQL);
	$TeamLayout = ReadFromDB ($XQuery);

	return array (
		'actionText' => 'became a fan of <a href="/live/content.php?Item_ID='.$TeamLayout['Item_ID'].'&Session_ID='.$Session_ID.'">'.$TeamResult['TeamName_STRING'].'</a>',
		'thumbnailImage' => '/live/images/phase2_images/'.$TeamResult['TeamPhoto_STRING'], 
		'thumbnailBlank' => '/live/images/phase2/teamsilhouettes.gif', 
		'summaryHead' => $TeamResult['TeamName_STRING'], 
		'summaryBody' => 'Click to see the team\'s profile', 
		'summaryLink' => '/live/content.php?Item_ID='.$TeamLayout['Item_ID'].'&Session_ID='.$Session_ID, 
		'extraOptionText' => 'Become a Fan', 
		'extraOptionLink' => '/live/content.php?Item_ID='.$TeamLayout['Item_ID'].'&Session_ID='.$Session_ID, 
	);

}

$CustomContentLinks = array (
	array ("text" => "Tag this Content", "url" => "IMM-update-tag.php?", "width" => 0, "height" => 0, "permission" => PROFILE_ADMINISTRATOR)
);

//define ("AuthoriseTrustedSource", "AuthoriseLDAP");
