...
function selection (l, u, p) {
	...
	//--- Creates the elements required for a selection
		...
		for (var i = 0; i < listEl.length; i++) {
			listEl[i].innerHTML = "<div class=\"item\" style=\"width: " + boxW + "px; height: " + boxH + "px\" onclick=\"top.selectionObjects['" + this.listField.id + "'].setActive (); top.lastSelection.click (this)\" onmouseover=\"this.className += ' over'\" onmouseout=\"this.className = this.className.toString ().replace (' over', '')\">" + listEl[i].innerHTML + "</div>";
		}
		...
		//--- Create filter HTML
		var html =
		"<strong>Find</strong> <input type=\"text\" id=\"" + this.listField.id + "_filter\" name=\"" + this.listField.id + "_filter\" onkeyup=\"top.lastSelection.filter (this.value)\" onfocus=\"top.selectionObjects['" + this.listField.id + "'].setActive ()\" autocomplete=\"off\" /><br />" + 
		"<div class=\"selectFilters\" style=\"width: " + dialogW + "px\">" +
		...
		"<li id=\"" + this.listField.id + "_view_selected\" class=\"\"><a onclick=\"top.selectionObjects['" + this.listField.id + "'].setActive (); top.lastSelection.tab (this); top.lastSelection.filter ('selected'); return false;\" href=\"#\">Selected (<strong id=\"" + this.listField.id + "_view_selected_count\">0</strong>)</a></li>" +
			...
			case 'all':		listEl = this.listField.getElementsByTagName ('li');
							...
							for (var i = 0; i < listEl.length; i++) {
								if (listEl[i].id != this.listField.id + '_nothing_to_show') {
									nameField = listEl[i].getElementsByTagName ('span')[0];
